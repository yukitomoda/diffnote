...
/** Showing some of the lines a place leaves out: which place, and from where. */
export type Expand = (gap: number, where: 'top' | 'bottom' | 'all') => Promise<unknown>;
...
function Expander(props: ExpanderProps) {
  ...
  var busy = _[0];
  var setBusy = _[1];
  var go = function (where: 'top' | 'bottom' | 'all') {
    if (busy) return;
    setBusy(true);
...
export function DiffTable(props: TableProps) {
  ...
  file.hunks.forEach(function (hunk, hi) {
    ...
    hunk.rows.forEach(function (row, ri) {
      ...
      var colors = lib.shownIds(ids, ctx.byId, ctx.hideResolved).map(function (id) { var p = ctx.placements[id]; return p.kind === 'line' ? p.color : 0; });
      var begin = compose ? function (e: MouseEvent) {
...
export function SplitTable(props: TableProps) {
  var file = props.file;
  var ctx = props.ctx;
  var cover = useMemo(
    function () {
  ...
  var hidden = function (ids: string[]) {
  ...
  var lo = sel ? Math.min(sel.anchor, sel.to) : -1;
  var hi_ = sel ? Math.max(sel.anchor, sel.to) : -1;
  var idxOf = function (row: Row | null) { return row ? indexOf.get(row) : undefined; };
  var pickedCell = function (row: Row | null, side: Side) {
    if (!sel || sel.side !== side || !row) return '';
    var i = idxOf(row);
    var has = side === 'old' ? row.o != null : row.n != null;
    if (!has || i == null || i < lo || i > hi_) return '';
    return ' is-picked' + (i === lo ? ' is-picked-first' : '') + (i === hi_ ? ' is-picked-last' : '');
  };
  var begin = function (row: Row | null, side: Side) {
    // (Against another revision, only the new side is this revision's.)
    if (ctx.compare && side === 'old') return undefined;
    if (!compose || !row) return undefined;
    const at = idxOf(row);
    if (at == null) return undefined;
    return function (e: MouseEvent) {
      if (e.button !== 0) return;
      e.preventDefault();
      compose!.begin(ctx.rev, file.path, at, e.shiftKey, side);
    };
  };
  var cellKind = function (row: Row | null, side: Side) {
    if (!row) return 'empty';
  ...
  file.hunks.forEach(function (hunk, hi) {
    ...
    lib.pairRows(hunk.rows).forEach(function (pair, pi) {
      ...
      var bars = function (side: string[]) {
        return '--diffnote-bars: ' + lib.bars(lib.shownIds(side, ctx.byId, ctx.hideResolved).map(function (id) { var p = ctx.placements[id]; return p.kind === 'line' ? p.color : 0; }));
      };
      ...
      var cards = r ? lib.cardsOfRow(after, r) : [];
