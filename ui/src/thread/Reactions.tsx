...
export function Reactions(props: ReactionsProps) {
  ...
  return <div class="diffnote-reactions" data-diffnote-reactions>
    {list.map(function (r) {
      ...
      return actions
        ? <button type="button" key={r.emoji} class={'diffnote-reaction' + (mine ? ' is-mine' : '')} data-diffnote-reaction={r.emoji}
            aria-pressed={mine} title={tip} onClick={function () { actions!.react(c.id, r.emoji); }}>{r.emoji}<span>{r.authors.length}</span></button>
        : <span key={r.emoji} class="diffnote-reaction" data-diffnote-reaction={r.emoji} title={tip}>{r.emoji}<span>{r.authors.length}</span></span>;
    })}
    {actions && <EmojiButton side="left" name="react" label="＋" title={lib.m('ui.reaction.add_title')} buttonClass="diffnote-reaction diffnote-reaction--add"
...
interface EmojiButtonProps {
  ...
  /** How it looks where it is put: beside a comment, or over a box. */
  side?: 'left';
  buttonClass?: string;
  name?: string;
  title?: string;
...
export function EmojiButton(props: EmojiButtonProps) {
  ...
  useEffect(function () {
    ...
    var key = function (e: KeyboardEvent) { if (e.key === 'Escape') { e.stopPropagation(); setOpen(false); } };
    ...
    {open && <div class="diffnote-emoji__panel" data-diffnote-emoji-panel>
      ...
      <div class="diffnote-emoji__grid">
        {found.map(function (e) {
          return <button type="button" key={e[1]} class="diffnote-emoji__item" data-diffnote-emoji={e[1]} title={':' + e[1] + ': ' + e[2]}
            onClick={function () { pick(e[0]); }}>{e[0]}</button>;
        })}
      </div>
      {found.length === 0 && <p class="diffnote-emoji__none">{lib.m('ui.emoji.none_found')}</p>}
    </div>}
  </span>;
}
